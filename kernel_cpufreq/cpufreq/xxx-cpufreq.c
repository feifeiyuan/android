	...
	struct device_node *np_cpufreq_data, u32 *p_binning_data)
{
	...
	if ((dev == NULL && np_cpufreq_data == NULL) || p_binning_data == NULL) {
...
/* Initializes OPP tables based on old-deprecated(弃用) bindings */
...
static struct regulator *xxx_volt_share_reg(struct xxx_cpufreq_driver_data *c)
{
	...
	for (cluster = 0; cluster < SPRD_CPUFREQ_MAX_MODULE; cluster++)
		if ((c->volt_share_masters_bits >> cluster) & 0x1) {
			...
			if (cpufreq_datas[cluster] != NULL && cpufreq_datas[cluster]->reg) {
...
static struct mutex *xxx_volt_share_lock(struct xxx_cpufreq_driver_data *c)
{
	...
	for (cluster = 0; cluster < SPRD_CPUFREQ_MAX_MODULE; cluster++)
		if ((c->volt_share_masters_bits >> cluster) & 0x1) {
			P_DBG("check master cluster%u\n", cluster);
			if (cpufreq_datas[cluster] != NULL &&cpufreq_datas[cluster]->volt_lock) {
	...
	struct xxx_cpufreq_driver_data *cpufreq_data, unsigned int idx, bool force)
{
	...
	// idx和cpufreq_data->freqvolts这个又应该有一个怎么样的关系呢？
	if (!cpufreq_data || (cpufreq_data && idx >= cpufreq_data->freqvolts)) {
		P_ERR("invalid cpufreq_data/idx%u, returning\n", idx);
		return -ENODEV;
	}

	P_DBG("setting target for cluster %d, freq idx %u freqvolts %u\n", cpufreq_data->cluster, idx, cpufreq_data->freqvolts);

	// 时钟和电源是必不可少的东西
	if (IS_ERR(cpufreq_data->clk) ||cpufreq_data->clk == NULL ||IS_ERR(cpufreq_data->reg) || cpufreq_data->reg == NULL) {
	...
	P_DBG("cluster%u scaling from %lu MHz, %ld mV, --> %lu MHz, %ld mV\n", cpufreq_data->cluster, old_freq_hz / 1000000, (volt_old > 0) ? volt_old / 1000 : -1,
				...
				unsigned int idx)
{
	...
	/*
	  ...
	  *so return 0 here, reject(拒绝) changing freq
	*/
...
static int xxx_cpufreq_init_slaves(struct device_node *np_host)
{
	...
	for (i = 0; i < SPRD_CPUFREQ_MAX_MODULE; i++) {
		...
		if (cluster < SPRD_CPUFREQ_MAX_CLUSTER || cluster >= SPRD_CPUFREQ_MAX_MODULE) {
		...
		of_property_read_u32(np, "volt-share-hosts-bits", &c->volt_share_hosts_bits);
		of_property_read_u32(np, "volt-share-masters-bits",	&c->volt_share_masters_bits);
		of_property_read_u32(np, "volt-share-slaves-bits",	&c->volt_share_slaves_bits);
...
static int xxx_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	// topology拓扑，就是拓扑结构的意思
	// policy->cpu是当前管理的cpu
	// topology_physical_package_id获取的是物理cpu的个数
	if (topology_physical_package_id(policy->cpu)>= SPRD_CPUFREQ_MAX_CLUSTER) {
		P_ERR("cpu%d in invalid cluster %d\n", policy->cpu, topology_physical_package_id(policy->cpu));
		return -ENODEV;
	}
	...
	c->clk_low_freq_p = of_clk_get_by_name(np, LOW_FREQ_CLK_PARENT);
	if (IS_ERR(c->clk_low_freq_p)) {
	...
	ret = dev_pm_opp_of_add_table_binning(is_big_cluster(cpu),cpu_dev, np, c);
...
static int  xxx_cpufreq_probe(struct platform_device *pdev)
{
	...
	//of_parse_phandle应该是解析设备树的函数
	np = of_parse_phandle(cpu_np, "cpufreq-data", 0);
	np1 = of_parse_phandle(cpu_np, "cpufreq-data-v1", 0);
	// 实际上解析的是cpufreq-data或者是cpufreq-data-v1
	if (!np && !np1) {
		P_ERR("failed to find cpufreq-data for cpu%d\n", cpu);
		of_node_put(cpu_np);
		// add by me
	...
	// 如果配置了cpufreq-data-v1则直接使用cpufreq-data-v1
	if (np1)
		np = np1;
...
put_np:
	...
	/* ret is not zero? we encountered(遇到) an error.
	 * return failure/probe deferred(延缓)
	*/
	if (ret)
		return ret;
