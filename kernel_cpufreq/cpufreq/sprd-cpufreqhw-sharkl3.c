...
#define FCFG_REFIN_MHZ ((long long)26)
#define FCFG_NINT(fvco_khz) \
((long long)(fvco_khz * 1000) / (FCFG_REFIN_MHZ * 1000000))

#define FCFG_KINT_REMAINDER(fvco_khz) \
((long long)(((fvco_khz * 1000) - \
...
#define FCFG_KINT_DIVISOR ((long long)(FCFG_REFIN_MHZ * 100))
...
#define VAL2REG_DCDC0_VTUNE_2721(to_uv)  \
VAL2REG_DCDC(to_uv, dcdc_desc[0].min_uv, dcdc_desc[0].step_uv, \
SCALE_TAB_VTUNE_MSK)

#define VAL2REG_DCDC0_VTUNE_FAN53(to_uv) \
VAL2REG_DCDC_FAN5355(to_uv, dcdc_fan53[0].min_uv, dcdc_fan53[0].step_uv, \
SCALE_TAB_VTUNE_MSK)

#define VAL2REG_DCDC0_VTUNE(to_uv) VAL2REG_DCDC0_VTUNE_FAN53(to_uv)

#define VAL2REG_DCDC1_VTUNE(to_uv) \
VAL2REG_DCDC(to_uv, dcdc_desc[0].min_uv, dcdc_desc[0].step_uv, \
SCALE_TAB_VTUNE_MSK)

#define REG2VAL_DCDC0_VTUNE(reg_uv) \
REG2VAL_DCDC(reg_uv, dcdc_desc[0].min_uv, dcdc_desc[0].step_uv, \
dcdc_desc[0].hide_offset_uv, SCALE_TAB_VTUNE_MSK)

#define REG2VAL_DCDC1_VTUNE(reg_uv) \
REG2VAL_DCDC(reg_uv, dcdc_desc[1].min_uv, dcdc_desc[1].step_uv, \
dcdc_desc[1].hide_offset_uv, SCALE_TAB_VTUNE_MSK)

/*channel0:little core*/
#define HW_DVFS_TAB_CLUSTER0(hz, uv, idx_freq) \
(VAL2REG_DCDC0_VTUNE(uv) | \
VAL2REG_FCFG((((hz) <= 768000000 ? 1 : 0) << 3) | idx_freq) | \
VAL2REG_FSEL((hz) <= 768000000 ? 0x02 : 0x06))

/*channel1:big core*/
#define HW_DVFS_TAB_CLUSTER1(hz, uv, idx_freq) \
(VAL2REG_DCDC1_VTUNE(uv) | \
VAL2REG_FCFG((((hz) <= 768000000 ? 1 : 0) << 3) | idx_freq) | \
VAL2REG_FSEL((hz) <= 768000000 ? 0x02 : 0x07))

/*channel2:fcm*/
#define HW_DVFS_TAB_CLUSTER2(hz, uv, idx_freq) \
(VAL2REG_DCDC1_VTUNE(uv) | \
VAL2REG_FCFG((((hz) <= 768000000 ? 1 : 0) << 3) | idx_freq) | \
...
static struct sprd_cpufreqhw_reg dcdc_desc[2] = {
...
static struct sprd_cpufreqhw_reg dcdc_fan53[2] = {
	{603000 + 602822, 12826, 603000, 1411000, 1000,
	 REG_DCDC0_VOL_CTL_FAN5355,
	 DCDC0_VOL_CTL_MSK_FAN5355},
	{603000 + 602822, 12826, 603000, 1411000, 1000,
	 REG_DCDC0_VOL_CTL_FAN5355,
	 DCDC0_VOL_CTL_MSK_FAN5355},
};
...
struct sprd_cpufreqhw {
	...
	spinlock_t lock[CPUFREQHW_CHNL_MAX];
	unsigned int chnl_in_i2c[CPUFREQHW_CHNL_MAX];
#endif
...
int sprd_cpufreqhw_try_lock(unsigned int cluster)
{
	...
	if (atomic_read(&cpufreqhw->dvfs_state[cluster]) !=
		...
		atomic_set(&cpufreqhw->dvfs_state[cluster],
			   CPUFREQHW_STATE_RUNNING);
#ifdef SPRD_CPUFREQ_I2C_LOCK
		if (cpufreqhw->chnl_in_i2c[cluster] &&
		cpufreqhw->i2c_client != NULL) {
			P_DBG_INT("CHNL%d i2c_lock_adapter\n", cluster);
			i2c_lock_adapter(cpufreqhw->i2c_client->adapter);
		}
#endif
	} else
		ret = -EBUSY;
...
int sprd_cpufreqhw_try_lock_big_scu(void)
{
	int ret = 0;
	...
	spin_lock_irqsave(
		&cpufreqhw->lock[CPUFREQHW_CHNL01],
		flags);
#endif
	if (atomic_read(&cpufreqhw->dvfs_state[CPUFREQHW_CHNL01]) !=
		...
		atomic_set(&cpufreqhw->dvfs_state[CPUFREQHW_CHNL02],
			   CPUFREQHW_STATE_RUNNING);
#ifdef SPRD_CPUFREQ_I2C_LOCK
		if (cpufreqhw->chnl_in_i2c[CPUFREQHW_CHNL01] &&
		cpufreqhw->i2c_client != NULL) {
			P_DBG_INT("CHNL%d i2c_lock_adapter\n",
				CPUFREQHW_CHNL01);
			i2c_lock_adapter(cpufreqhw->i2c_client->adapter);
		}
		if (cpufreqhw->chnl_in_i2c[CPUFREQHW_CHNL02] &&
		cpufreqhw->i2c_client != NULL) {
			P_DBG_INT("CHNL%d i2c_lock_adapter\n",
				CPUFREQHW_CHNL02);
			i2c_lock_adapter(cpufreqhw->i2c_client->adapter);
...
void sprd_cpufreqhw_unlock(unsigned int cluster)
{
#ifdef SPRD_CPUFREQ_I2C_LOCK
	unsigned long flags;
...
static int sprd_cpufreqhw_init_param(struct device_node *np)
{
	...
	of_property_read_u32(np, "sprd,chnl-in-i2c",
		&cfg[0]);
	cpufreqhw->chnl_in_i2c[0] = (cfg[0] & 0x00f);
	cpufreqhw->chnl_in_i2c[1] = (cfg[0] & 0x0f0) >> 4;
	cpufreqhw->chnl_in_i2c[2] = (cfg[0] & 0xf00) >> 8;
	P_DBG("sprd,chnl-in-i2c %u->%u,%u,%u\n", cfg[0],
		cpufreqhw->chnl_in_i2c[0],
		cpufreqhw->chnl_in_i2c[1],
		cpufreqhw->chnl_in_i2c[2]);
#endif
	/*configure DCDC MAP*/
	cfg[0] = 0;
...
static int hwdvfs_trigger_lit(unsigned int scalecode00, bool sync, bool force)
{
	...
	if (sprd_cpufreqhw_try_lock(CPUFREQHW_CHNL00)) {
		...
		if (cpufreqhw->busy[CPUFREQHW_CHNL00] &&
		    time_after(jiffies,
			       cpufreqhw->busy[CPUFREQHW_CHNL00])) {
			P_DBG("CHNL0 busy expired!\n");
			sprd_cpufreqhw_unlock(CPUFREQHW_CHNL00);
		}
		return 0;
	}
#endif
...
static int hwdvfs_trigger_big(unsigned int scalecode01, bool sync, bool force)
{
	...
	if (sprd_cpufreqhw_try_lock(CPUFREQHW_CHNL01)) {
		...
		if (cpufreqhw->busy[CPUFREQHW_CHNL01] &&
		    time_after(jiffies,
			       cpufreqhw->busy[CPUFREQHW_CHNL01])) {
			P_DBG("CHNL1 busy expired!\n");
			sprd_cpufreqhw_unlock(CPUFREQHW_CHNL01);
		}
		return 0;
	}
#endif
...
static int hwdvfs_trigger_scu(unsigned int scalecode02, bool sync, bool force)
{
	...
	if (sprd_cpufreqhw_try_lock(CPUFREQHW_CHNL02)) {
		...
		if (cpufreqhw->busy[CPUFREQHW_CHNL02] &&
		    time_after(jiffies,
			       cpufreqhw->busy[CPUFREQHW_CHNL02])) {
			P_DBG("CHNL2 busy expired!\n");
			sprd_cpufreqhw_unlock(CPUFREQHW_CHNL02);
		}
		return 0;
	}
#endif
				  ...
				  bool force)
{
	...
	if (scalecodeing == scalecode01
	    && cpufreqhw->triggered[CPUFREQHW_CHNL01]
	    && !force) {
		P_ERR("CHNL1 CHNL2 reject same idx-%u int-%u\n", scalecode01,
		      REG2VAL(regval1, BIT_DVFS_CTRL_IRQ_DONE_CHNL01));
		return 0;
	}

	if (sprd_cpufreqhw_try_lock_big_scu()) {
		P_ERR("CHNL1 CHNL2 busy! cur-%u ing-%u to-%u\n",
		...
		if (cpufreqhw->busy[CPUFREQHW_CHNL01] &&
		    time_after(jiffies,
			       cpufreqhw->busy[CPUFREQHW_CHNL01])) {
			P_DBG("CHNL1 CHNL2 busy expired!\n");
			sprd_cpufreqhw_unlock(CPUFREQHW_CHNL01);
			sprd_cpufreqhw_unlock(CPUFREQHW_CHNL02);
		}
		return 0;
	...
	if (sync) {
		...
		if (i >= RETRY_MAX) {
			regval1 = _dvfs_rd_(REG_DVFS_CTRL_STS_CHNL01);
			P_DBG("CHN01=%d fail (0x%x), DONE=0x%x,SCALE=0x%x\n",
			      scalecode01, regval1,
			      REG2VAL(regval1, BIT_DVFS_CTRL_IRQ_DONE_CHNL01),
			      REG2VAL(regval1,
				BIT_DVFS_CTRL_DONE_SCL_CHNL01(0xf)));
			   ...
			   unsigned long u_volt, int idx_volt)
{
	...
	case CPUFREQHW_CHNL00:
		...
		if (hz_freq > 768000000)
			...
			idx_freq) * 0x4,
			MPLL_HW_DVFS_TAB_CLUSTER0(hz_freq / 1000));
	...
	case CPUFREQHW_CHNL01:
		...
		if (hz_freq > 768000000)
			...
			(cluster * MPLL_HW_DVFS_EACH_NUM +
			idx_freq) * 0x4,
			MPLL_HW_DVFS_TAB_CLUSTER1(hz_freq / 1000));
	...
	case CPUFREQHW_CHNL02:
		...
		if (hz_freq > 768000000)
			...
			(cluster * MPLL_HW_DVFS_EACH_NUM +
			idx_freq) * 0x4,
			MPLL_HW_DVFS_TAB_CLUSTER2(hz_freq / 1000));
...
static int sprd_cpufreqhw_probe(struct platform_device *pdev)
{
	...
	spin_lock_init(&cpufreqhw->lock[CPUFREQHW_CHNL00]);
	spin_lock_init(&cpufreqhw->lock[CPUFREQHW_CHNL01]);
	spin_lock_init(&cpufreqhw->lock[CPUFREQHW_CHNL02]);
#endif
	cpufreqhw->probed = true;
