 ...
 *
 * This program is free software; you can redistribute(重新分配) it and/or modify
...
// DEFINE_RWLOCK宏来初始化读写锁
static DEFINE_RWLOCK(cpufreq_driver_lock);

// DEFINE_MUTEX宏来定义和初始化互斥体
...
{
	// 定义了target_index或者target时通过governor来做调节
 ...
 */
 ...
 // BLOCKING_NOTIFIER_HEAD这个应该和内核通知链notifier有关系
...
static inline u64 get_cpu_idle_time_jiffy(unsigned int cpu, u64 *wall)
{
	...
	busy_time += kcpustat_cpu(cpu).cpustat[CPUTIME_NICE];//这里将nice值和user分开统计了
		...
		struct cpufreq_freqs *freqs, int transition_failed)
{
	...
	// 如果失败则回到之前的配置，同时发送两个transition通知
...
wait:
	...
	// 实际上发送CPUFREQ_PRECHANGE这个transition通知
	cpufreq_notify_transition(policy, freqs, CPUFREQ_PRECHANGE);
}
...
// 这个是新加的
static int cpufreq_add_policy_cpu(struct cpufreq_policy *policy, unsigned int cpu)//cpu应该是当前cpu
{
	int ret = 0;

	/* Has this CPU been taken care of already? */
	// 如果cpu已经被关联到policy中了，则直接返回
...
static int cpufreq_online(unsigned int cpu)
{
	...
	// 将同一个policy管理的cpu掩码放到policy的cpus里面去
...
void cpufreq_suspend(void)
{
	...
	for_each_active_policy(policy) {
		...
		else if (cpufreq_driver->suspend && cpufreq_driver->suspend(policy))
					...
					unsigned int event)
{
	...
	/* Only must be defined when default governor is known to have latency
	   restrictions(限制), like e.g. conservative or ondemand.
	...
	*/
	...
	if (policy->governor->max_transition_latency &&
	    ...
	    policy->governor->max_transition_latency) {//所以软件的latency应该小于等于硬件的latency了
		if (!gov)
			return -EINVAL;
		else {
 ...
 */
 // 设置新的policy. 主要是更新max/min, 若governor发生变化了, 
 // 则先stop old governor, 再start new governor. 同时回调cpufreq_policy_notifier_list上的通知事件
static int cpufreq_set_policy(struct cpufreq_policy *policy,
				struct cpufreq_policy *new_policy)
{
	...
	// 从旧的policy的cpuinfo(硬件)上继承cpuinfo
	...
	*/
	...
	// 确认policy是否合理, 调整policy.max/min
	ret = cpufreq_driver->verify(new_policy);
	if (ret)
		return ret;

	/* adjust if necessary - all reasons */
	// 通过内核通知链向对应的模块发送policy通知
...
int cpufreq_update_policy(unsigned int cpu)
{
	...
	// 使用policy的user_policy来update_policy
	...
	if (cpufreq_driver->get && !cpufreq_driver->setpolicy) {
		...
		if (!policy->cur) {
			...
			// 若实际频率与缓存的不一致, 则发送PRECHANGE /  POSTCHANGE通知
			// 在POSTCHANGE中, policy.cur会被更新
...
static int __init cpufreq_core_init(void)
{
	...
	// syscore是Linux kernel定义的一个framework
	// 将驱动注册到syscore中
